---
layout: page
title: About
permalink: /about/
---

I'm William Thissell, an Aerospace Engineering student at the University of Virginia,
completing my degree on an accelerated three-year track. I excel at working through
long, challenging, quantitative problems, and I'm always willing to get hands-on with a
project of my own to see it through to a final product.

Alongside my engineering coursework and projects, I have been pursuing consulting,
which presents a different way of thinking with a more structured approach to problem
solving.

## At a Glance

<dl class="facts">
  <dt>Education</dt>
  <dd>B.S. Aerospace Engineering, University of Virginia<br>Expected May 2028 · GPA 3.83</dd>
  <dt>Clubs</dt>
  <dd>UVA Rocketry, Fixed Income &amp; Derivatives</dd>
  <dt>Leadership</dt>
  <dd>Eagle Scout; Senior Patrol Leader over a troop of 25 to 50 Scouts; Crew Leader on a 10-day Philmont trek; Team Captain at South Block after two promotions in four months</dd>
  <dt>Honors</dt>
  <dd>FIRST Robotics Competition World Championship competitor (2024)</dd>
  <dt>Certifications</dt>
  <dd>Certified SOLIDWORKS Associate, PwC Management Consulting Simulation</dd>
  <dt>Tools</dt>
  <dd>SOLIDWORKS, AutoCAD, CFD, FEA, MATLAB, Python, Java, Claude Code, Arduino</dd>
</dl>

## Interests

- Aerospace design and propulsion
- Applied AI, particularly in finance
- Management consulting and strategy

## Contact

- Email: [{{ site.author.email }}](mailto:{{ site.author.email }})
{% if site.linkedin_username and site.linkedin_username != "" %}- LinkedIn: [{{ site.author.name }}](https://www.linkedin.com/in/{{ site.linkedin_username }}/)
{% endif %}
